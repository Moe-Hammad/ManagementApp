 ...
 * useChatView
 * --------------------------------------
 * Bündelt die Chat-spezifische Logik für den Detail-Screen:
 * - Raum + Partner ermitteln
 * - Nachrichten laden / Polling
 * - WebSocket-Subscribe für neue Messages
 * - Nachricht senden
 */
export function useChatView(chatId?: string) {
  ...
  // Polling (Fallback für verpasste WS-Events)
  useEffect(() => {
